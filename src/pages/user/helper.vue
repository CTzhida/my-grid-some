    ...
    <el-card shadow="never" style="min-height: 600px;margin-bottom: 20px;">
      <el-collapse v-model="active" accordion>
        <el-collapse-item title="如何部署" name="deploy">
          <el-carousel :autoplay="false" trigger="click" height="352px" @change="deployChange">
          ...
          <el-steps :active="deploy.active" :process-status="deploy.status" align-center style="margin-top: 20px;">
            <el-step title="克隆项目" description="https://github.com/GitHub-Laziji/GitHub-Laziji.github.io"/>
            <el-step title="修改配置" description="将 /static/configuration.json 中的 githubUsername  改为自己的"/>
            <el-step title="查看效果" description="https://用户名.github.io 就是你的博客了"/>
          </el-steps>
        </el-collapse-item>
        <el-collapse-item title="如何使用" name="use">
          <el-carousel :autoplay="false" trigger="click" height="352px" @change="useChange">
          ...
          <el-steps :active="use.active" :process-status="use.status" align-center style="margin-top: 20px;">
            <el-step title="获取Token" description="在GitHub上获取调用API所需的Token"/>
            <el-step title="登录" description="将获取的Token绑定上"/>
            <el-step title="发个博客" description="点击博客列表右上角的发布博文按钮"/>
          </el-steps>
        </el-collapse-item>
